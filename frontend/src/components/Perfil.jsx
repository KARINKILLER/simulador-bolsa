...
const Perfil = () => {   
    ...
    useEffect(() => {
        ...
        const verificarSesion = async () => {
            try {
                const response = await fetch("https://shivering-adriena-backendtfg-b6859741.koyeb.app/session-status", {
                    method: "GET",
                    credentials: "include"
                });
        ...
        const cargarPerfil = async () => {
            try {
                ...
                const response = await fetch('https://shivering-adriena-backendtfg-b6859741.koyeb.app/cargar-activos-perfil', {
                    credentials: 'include'
                });
                const response2 = await fetch('https://shivering-adriena-backendtfg-b6859741.koyeb.app/cargar-transacciones-perfil', {
                    credentials: 'include'
                });
    ...
    const reinicio = async () => {
        const response = await fetch('https://shivering-adriena-backendtfg-b6859741.koyeb.app/reinicio', {method: 'POST', credentials: 'include'});
        if (!response.ok) throw new Error(`Error: ${response.status}`);
        setModalOpen(false); 
