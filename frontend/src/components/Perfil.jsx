...
const Perfil = () => {   
    ...
    useEffect(() => {
        ...
        const cargarPerfil = async () => {
            try {
                ...
                const response = await fetch('https://simulador-bolsa-05g9.onrender.com/cargar-activos-perfil', {
                    credentials: 'include'
                });
                const response2 = await fetch('https://simulador-bolsa-05g9.onrender.com/cargar-transacciones-perfil', {
                    credentials: 'include'
                });
    ...
    const reinicio = async () => {
        const response = await fetch('https://simulador-bolsa-05g9.onrender.com/reinicio', {method: 'POST', credentials: 'include'});
        if (!response.ok) throw new Error(`Error: ${response.status}`);
        setModalOpen(false); 
